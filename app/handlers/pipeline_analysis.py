...
async def handle_pipeline(payload: dict) -> None:
    ...
    try:
        ...
        if webhook_config and webhook_config.get("enabled"):
            await google_chat.send_event(
                event_type="pipeline_analysis", body=analysis,
                title=f"Pipeline #{pipeline_id} failed on {ref}",
                url=pipeline_url, project_name=project_name,
    ...
    try:
        await db.save_event(
            event_type="pipeline_analysis",
            project_id=project_id,
            project_name=project_name,
            ref_id=str(pipeline_id),
            ref_url=pipeline_url,
